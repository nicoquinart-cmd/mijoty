...
create policy "recipe ingredients read" on public.recipe_ingredients for select using(exists(select 1 from public.recipes r where r.id=recipe_id and (r.household_id is null or public.is_household_member(r.household_id))));
create policy "recipe ingredients write" on public.recipe_ingredients for all using(exists(select 1 from public.recipes r where r.id=recipe_id and r.household_id is not null and public.is_household_member(r.household_id))) with check(exists(select 1 from public.recipes r where r.id=recipe_id and r.household_id is not null and public.is_household_member(r.household_id)));
create policy "recipe steps read" on public.recipe_steps for select using(exists(select 1 from public.recipes r where r.id=recipe_id and (r.household_id is null or public.is_household_member(r.household_id))));
create policy "recipe steps write" on public.recipe_steps for all using(exists(select 1 from public.recipes r where r.id=recipe_id and r.household_id is not null and public.is_household_member(r.household_id))) with check(exists(select 1 from public.recipes r where r.id=recipe_id and r.household_id is not null and public.is_household_member(r.household_id)));
